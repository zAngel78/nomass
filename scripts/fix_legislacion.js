...
class LegislacionFixer {
  constructor() {
    this.dataDir = path.join(__dirname, '../data');
    this.legislacionFiles = [
      'questions_legislacion_normal.json',
      'questions_legislacion_general.json'
    ];
    this.errors = [];
  }
  ...
  fixQuestion(question, index) {
    try {
      ...
      const fixed = {
        id: question.id || `legislacion_${Date.now()}_${index}`,
  ...
  async processFile(filename) {
    ...
    try {
      ...
      const backupPath = path.join(this.dataDir, `${filename}.backup_legislacion.${Date.now()}`);
  ...
  async fixAllFiles() {
    console.log('🚀 Iniciando corrección de formato para Legislación...\n');

    const results = [];

    for (const filename of this.legislacionFiles) {
...
if (require.main === module) {
  const fixer = new LegislacionFixer();
  fixer.fixAllFiles()
    .then(() => {
...
module.exports = LegislacionFixer;